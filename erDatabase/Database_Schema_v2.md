...
> 3. เพิ่มตาราง `station` แยก แล้วเปลี่ยน field ที่เคยเป็น string/int เปล่า ๆ ให้เป็น FK ไปยัง `station`
> 4. เปลี่ยนตาราง `penalty_strike` (ของเอกพล) เป็น **`favorite`** (Favorite Bicycles & Stations) ตาม scope งานใหม่
> 5. หมายเหตุเรื่อง cross-service sync ระหว่าง Monolith (Django) กับ Client-Server (FastAPI) ยังคงไว้ท้ายเอกสาร
...
CREATE TYPE group_ride_status      AS ENUM ('open', 'full', 'cancelled', 'completed');
CREATE TYPE group_ride_member_role AS ENUM ('leader', 'member');
```
...
erDiagram
    ...
    GROUP_RIDE_MEMBER {
        int id PK
        int group_ride_id FK
        int user_id FK
        group_ride_member_role role
        datetime joined_at
        datetime left_at
...
| `bicycle` | จักรยานแต่ละคัน | `current_location` (string) → เปลี่ยนเป็น `current_station_id` (FK → `station`), `status` → ENUM `bicycle_status` |
| `maintenance` | บันทึกแจ้งซ่อม/บำรุงรักษา | `type`, `status` → ENUM (`maintenance_type`, `maintenance_status`) |
| `station` *(ตารางใหม่)* | จุดจอด/สถานีจักรยาน | เพิ่มใหม่ตามข้อควรพิจารณาข้อ 3 เพื่อรองรับ FK จากหลายตาราง |
...
| `return_record` | บันทึกการคืนจักรยาน | `return_station_id` → เปลี่ยนเป็น FK จริงไปยัง `station`, `status` → ENUM |

**รายละเอียด CRUD ของ `favorite`:**
- **Create** — `add_favorite(user_id, target_type, bicycle_id/station_id, nickname)`
...
`target_type` (`favorite_target_type`) ใช้กำหนดว่ารายการนี้อ้างถึง `bicycle_id` หรือ `station_id` — ควรบังคับด้วย `CHECK constraint` ว่ามีค่าแค่ฝั่งเดียว (อีกฝั่งเป็น NULL) ตาม `target_type`
...
**รายละเอียด CRUD ของ `group_ride` (ตามเอกสารหัวข้ออาจารย์):**
- **Create (สร้างกลุ่ม)** — ผู้ใช้ตั้งกลุ่มปั่นใหม่ ระบุชื่อกลุ่ม จุดหมายปลายทาง เวลานัดหมาย และจำนวนสมาชิกที่เปิดรับ → หัวหน้ากลุ่ม (`created_by`) จะถูกบันทึกเป็นสมาชิกคนแรกใน `group_ride_member` (role=`leader`) และ `current_members=1`, สถานะ `open`
- **Read (ค้นหา/ดูกลุ่ม)** — ดูรายชื่อกลุ่มปั่นที่สถานะ `open` (รอบตัว) เพื่อดูรายละเอียดและตัดสินใจเข้าร่วม; `GET /api/group-rides?status=open`
- **Update (เข้าร่วม/แก้ไขกลุ่ม)** —
  - ผู้ใช้ทั่วไปกด "เข้าร่วมกลุ่ม" → เพิ่มแถวใน `group_ride_member`, `current_members += 1`, และเปลี่ยนสถานะเป็น `full` หากเต็ม
  - หัวหน้ากลุ่มแก้ไขเวลานัดหมาย/จุดหมายปลายทาง/จำนวนสมาชิกได้
- **Delete (ยกเลิกกลุ่ม/ออกจากกลุ่ม)** —
  - หัวหน้ากลุ่มกดยกเลิกกลุ่ม → สถานะเปลี่ยนเป็น `cancelled`
  - สมาชิกกดออกจากกลุ่ม → ตั้ง `left_at`, `current_members -= 1`, กลับเป็น `open` เพื่อคืนโควตาให้คนอื่น

---

## ⚠️ หมายเหตุ Cross-service (Monolith vs FastAPI)

ระบบแบ่งเป็น Django Monolith (ปิยะพงษ์, วีรพันธ์) และ FastAPI/React (ณธิดา, เอกพล, ชัยอนันต์) — field ที่อ้างข้ามฝั่ง เช่น `user_id`, `bicycle_id`, `station_id` ที่ฝั่ง FastAPI ต้องอ้างถึงข้อมูลจากฝั่ง Django ควรพิจารณา 2 แนวทาง:

1. **Shared database** — ทั้งสองฝั่งต่อ Postgres instance เดียวกัน (FK ตรงใช้งานได้ปกติ ง่ายสุดสำหรับ mock/prototype)
2. **แยก database + sync ผ่าน API** — แต่ละฝั่งมี DB ของตัวเอง แล้วอ้างอิงกันด้วย `id` เปล่า ๆ (ไม่มี FK constraint ข้าม DB จริง) ต้องเรียก internal API เพื่อ validate ว่า `user_id`/`bicycle_id` มีอยู่จริงก่อน insert
